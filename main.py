...
#     preprocess_csv('tweets_data.csv', 'tweets_data.csv', test_file=False)


final = []
words = []
with open('tweets_data.csv', encoding="utf8") as file:

    reader = csv.reader(file)
    for row in reader:
        word2 = row[1]
        word2 = preprocess_word(word2)
        word2 = handle_emojis(word2)
        word2 = preprocess_tweet(word2)
        words.append(word2)
        word2 = word2.replace(" ", "")
        counter = word2.count(',')
        if counter > 0:
